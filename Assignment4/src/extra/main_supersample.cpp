...
// New function, supersampling 4x --> Every 4x4 group of pixels is sampled in one pixel 
FrameBuffer scale_down_4x(const FrameBuffer& fb)
{
	// The size of the framebuffer must be a multiple of 4
	// Assert: Directive to use only when we compile in debug mode
	// It check if the condition id true, the condition is boolean and we want that it is true. 
	// If we compile in release mode the asserion are totally ignored. 
	assert(fb.rows() % 4 == 0);
	assert(fb.cols() % 4 == 0);
		...
		for (unsigned j=0;j<out.cols();j++)
		{
			...
			for (unsigned ii=0;ii<4;ii++)
				for (unsigned jj=0;jj<4;jj++)
					// Average of the corrisponding group
